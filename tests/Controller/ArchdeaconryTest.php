...
use App\DataFixtures\ArchdeaconryFixtures;
use App\Repository\ArchdeaconryRepository;
use Nines\UserBundle\DataFixtures\UserFixtures;
use Nines\UtilBundle\Tests\ControllerBaseCase;
use Symfony\Component\HttpFoundation\Response;

class ArchdeaconryTest extends ControllerBaseCase {
    ...
    public function testAnonSearch() : void {
        $repo = $this->createMock(ArchdeaconryRepository::class);
        $repo->method('searchQuery')->willReturn([$this->getReference('archdeaconry.1')]);
        $this->client->disableReboot();
        $this->client->getContainer()->set('test.' . ArchdeaconryRepository::class, $repo);

        $crawler = $this->client->request('GET', '/archdeaconry/search');
        $this->assertSame(self::ANON_RESPONSE_CODE, $this->client->getResponse()->getStatusCode());
        if (self::ANON_RESPONSE_CODE === Response::HTTP_FOUND) {
            // If authentication is required stop here.
            return;
    ...
    public function testAnonNewPopup() : void {
        $crawler = $this->client->request('GET', '/archdeaconry/new_popup');
        $this->assertSame(Response::HTTP_FOUND, $this->client->getResponse()->getStatusCode());
        $this->assertTrue($this->client->getResponse()->isRedirect());
    }
    ...
    public function testAdminNewPopup() : void {
        $this->login('user.admin');
        $formCrawler = $this->client->request('GET', '/archdeaconry/new_popup');
        $this->assertSame(Response::HTTP_OK, $this->client->getResponse()->getStatusCode());
    ...
    public function testAdminDelete() : void {
        $repo = self::$container->get(ArchdeaconryRepository::class);
